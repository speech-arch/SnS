...
const filteredItems = computed(() => {
  let rawItems = items;
  if (rawItems && typeof rawItems === 'object' && 'value' in rawItems) {
    rawItems = rawItems.value;
  }
  if (!Array.isArray(rawItems)) return [];
  return rawItems.filter(item => item.label?.toLowerCase() !== 'about');
});
</script>
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 w-full">
      <Menubar :model="filteredItems" class="card items-center hidden sm:flex w-full">
        ...
        <template #item="{ item, root, hasSubmenu, props }">
          <RouterLink
            v-if="!item.items && item.label.toLowerCase() !== 'places' && item.label.toLowerCase() !== 'create' && item.label.toLowerCase() !== 'about'"
